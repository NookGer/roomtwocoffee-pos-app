...
export default function App() {
  ...
  function confirmPay(lastCart,lastTotal){
    const rcv=parseInt(modal.received||"0",10); if(rcv<lastTotal)return;
  ...
  return (
    <div style={{fontFamily:"'Sarabun','Noto Sans Thai',sans-serif",background:"#F5F0EA",minHeight:"100vh",display:"flex",flexDirection:"column",userSelect:"none"}}>
      ...
      {view==="manage"  && <ManageView data={data} persist={(nd,s)=>persist(nd,null,null,null,s)} onClearData={clearData}/>}
...
function Overlay({children,onClose,wide}){ return<div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(28,12,4,.58)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,backdropFilter:"blur(5px)"}}><div onClick={e=>e.stopPropagation()} style={{background:"#FFFCF8",borderRadius:20,padding:26,width:wide?580:390,maxHeight:"92vh",overflowY:"auto",boxShadow:"0 28px 72px rgba(0,0,0,.3)",border:"1px solid #E8D8C8"}}>{children}</div></div>; }
function AddBtn({children,onClick,color="#2C1810"}){ return<button onClick={onClick} style={{background:color,color:"#FFF",border:"none",borderRadius:10,padding:"9px 16px",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",gap:5}}><Plus size={13}/>{children}</button>; }
function SectionLabel({children}){ return<div style={{fontSize:12,color:"#8C7C6C",fontWeight:600,marginBottom:7}}>{children}</div>; }
...
function ManageView({data,persist,onClearData}){
  ...
  return (
    <div style={{flex:1,overflowY:"auto",padding:"20px 24px"}}>
      ...
      {/* ADD-ONS */}
      {tab==="addons"&&<div style={{display:"flex",flexDirection:"column",gap:8}}>
        ...
        {addons.map(ao=>(
          <div key={ao.id} style={{display:"flex",alignItems:"center",gap:10,background:"#FFF8F2",border:"1px solid #E8D8C8",borderRadius:13,padding:"11px 14px"}}>
            ...
            <div style={{flex:1}}><div style={{fontSize:14,fontWeight:600,color:"#2C1810"}}>{ao.name} <span style={{color:"#7941C8",fontWeight:700}}>+฿{ao.price}</span></div></div>
      ...
      <div style={{marginTop:32,paddingTop:24,borderTop:"1px solid #E4D4C0"}}>
        <div style={{fontSize:13,color:"#9C8C7C",marginBottom:10}}>⚠️ โซนอันตราย</div>
        <button onClick={()=>setIM({type:"confirm",icon:<AlertTriangle size={36} color="#C84B4B" style={{margin:"0 auto 12px"}}/>,msg:"ล้างประวัติการขาย?\n\nจะล้าง: orders, ledger (ยกเว้นตั้งค่าเริ่มต้น), cutoffs\nเมนูสินค้ายังอยู่ครบ",confirmLabel:"ล้างข้อมูล",confirmColor:"#C84B4B",onConfirm:onClearData})}
...
function ProdFormShell({initState,title,data,persist,onClose}){
  ...
  const addons=data.addons||[], freeOpts=data.freeOpts||[], discounts=data.discounts||[];
  const aV=()=>setVars(v=>[...v,{id:`v${uid()}`,name:"",price:""}]);
  const rV=id=>setVars(v=>v.filter(x=>x.id!==id));
  const uV=(id,f,val)=>setVars(v=>v.map(x=>x.id===id?{...x,[f]:val}:x));
  ...
  const save=()=>{
    ...
    const prod={id:initState.id||`p${uid()}`,categoryId:catId,name:name.trim(),color,image,unit,variants:vts,linkedAddons,linkedFreeOpts,linkedDiscounts,order:initState.order??data.products.filter(p=>p.categoryId===catId).length};
...
function PaymentModal({modal,setModal,cartTotal,onConfirm}){
  ...
  return(
    <div>
      ...
      <div style={{background:"#F5F0EA",borderRadius:12,padding:"12px 14px",fontSize:26,fontWeight:700,color:"#2C1810",textAlign:"right",marginBottom:12,minHeight:52}}>{disp?baht(parseInt(disp,10)):<span style={{color:"#C0B0A0",fontSize:18}}>ใส่จำนวนเงิน</span>}</div>
    ...
    <div style={{textAlign:"center",padding:"4px 0"}}>
      ...
      {showR&&order&&<div style={{borderRadius:12,overflow:"hidden",border:"1px solid #D4C4B0"}}>
        ...
        <div ref={receiptRef} style={{background:"#fff",color:"#000",fontFamily:"'Sarabun','Noto Sans Thai',sans-serif",padding:"20px 18px",textAlign:"center",width:"100%"}}>
          ...
          <div style={{textAlign:"left",fontSize:12,color:"#333",lineHeight:1.9}}><div>เลขที่บิล: <b>{order.orderNum?fmtNum(order.orderNum):`#${order.id?.slice(-4).toUpperCase()}`}</b></div><div>วันที่: {fmtDate(order.date)}</div><div>เวลา: {fmtTime(order.ts)}</div></div>
          <div style={{borderTop:"1px dashed #aaa",margin:"10px 0"}}/>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13,textAlign:"left"}}><thead><tr style={{borderBottom:"1px solid #ccc"}}><th style={{padding:"3px 0",fontWeight:600}}>รายการ</th><th style={{textAlign:"center",fontWeight:600}}>จำนวน</th><th style={{textAlign:"right",fontWeight:600}}>ราคา</th></tr></thead>
          <tbody>{order.items.map((item,i)=><tr key={i}><td style={{padding:"3px 0",lineHeight:1.5}}>{item.name} <span style={{color:"#666",fontSize:11}}>({item.variant})</span>{item.note&&<div style={{fontSize:10,color:"#888"}}>— {item.note}</div>}</td><td style={{textAlign:"center"}}>{item.qty} {item.unit||""}</td><td style={{textAlign:"right"}}>฿{(item.price*item.qty).toLocaleString()}</td></tr>)}</tbody></table>
          <div style={{borderTop:"1px solid #aaa",marginTop:8,paddingTop:8}}><div style={{display:"flex",justifyContent:"space-between",fontWeight:700,fontSize:15}}><span>ยอดรวม</span><span>฿{order.total?.toLocaleString()}</span></div><div style={{display:"flex",justifyContent:"space-between",fontSize:12,color:"#555",marginTop:3}}><span>รับเงิน</span><span>฿{order.received?.toLocaleString()}</span></div><div style={{display:"flex",justifyContent:"space-between",fontSize:12,color:"#555"}}><span>เงินทอน</span><span>฿{(order.change||0).toLocaleString()}</span></div></div>
          <div style={{borderTop:"1px dashed #aaa",margin:"10px 0"}}/>
